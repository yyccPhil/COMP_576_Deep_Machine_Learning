...
class NeuralNetwork(object):
    ...
    def actFun(z, type):
        ...
        if type == 'tanh':
            ...
            act = z * np.greater(z, 0).astype(int)
    ...
    def diff_actFun(z, type):
        ...
        diff_act = 0
        if type == 'tanh':
            diff_act = 1 - ((np.exp(z) - np.exp(-z)) / (np.exp(z) + np.exp(-z))) ** 2
        elif type == 'sigmoid':
            a = 1 / (1 + np.exp(-z))
            diff_act = a * (1 - a)
        elif type == 'relu':
            diff_act = np.greater(z, 0).astype(int)
    ...
    def feedforward(self, X, actFun):
        ...
        sumExp = np.exp(self.z2 - np.max(self.z2))
        self.probs = sumExp/np.sum(sumExp, axis = 1, keepdims = True)
